...
export const REQUIRED = {
  shopify: ['storeUrl','clientId','clientSecret','refreshToken','entity','fields'] as const,
  snowflake: ['account','username','password','database','schema','table','loadMode','key'] as const,
}
...
export const useSlots = create<State>((set, get) => ({
  ...
  isComplete: () => {
    const s = get().slots
    return REQUIRED.shopify.every(k => (s.shopify as any)[k]) &&
